 ...
 */
(function () {
    var toolkit = /modern/.test(window.location.pathname) ? 'modern'
        : 'classic';
    ...
    var scriptEls = document.getElementsByTagName('script'),
        ...
        themeName = (getQueryParam('theme') || 'triton').replace(
            /^([-A-Za-z]*)(.*)/, '$1'),
        ...
        themePackageDir, sdkRoot, d3JS, uxJS, chartsJS, themeOverrideJS,
        extPrefix, extPackagesRoot;
    ...
    extPackagesRoot = devMode ? (extDir + '/app') : extDir;
    if (devMode === 1) {
        extDir = extPackagesRoot;
    }

    uxJS = extPackagesRoot + '/packages/ux/' + toolkit + '/ux' + debugSuffix
        + '.js';
    chartsJS = extPackagesRoot + '/packages/charts/' + toolkit + '/charts'
        + debugSuffix + '.js';
    ...
    if (includeCSS) {
        loadCss(
            themePackageDir + 'resources/theme-' + themeName + '-all'
            + cssSuffix);
        if (toolkit === 'modern') {
            loadCss(
                extPackagesRoot + '/packages/charts/modern/modern-' + themeName
                + '/resources/charts-all' + cssSuffix);
        } else {
            loadCss(extPackagesRoot + '/packages/charts/' + toolkit + '/'
                + themeName
                + '/resources/charts-all' + cssSuffix);
        }
        loadCss(extPackagesRoot + '/packages/ux/' + toolkit + '/' + themeName
            + '/resources/ux-all' + cssSuffix);
        if (devMode === 2) {
            loadCss(sdkRoot + '/packages/d3/build/' + toolkit + '/' + themeName
                + '/resources/d3-all' + cssSuffix);
        }
    }
    ...
    if (toolkit === 'modern') {
        ...
        extPrefix = useDebug ? '/build/ext' : '/build/ext-all';
    }

    document.write(
        '<script type="text/javascript" src="' + extDir + extPrefix + rtlSuffix
        + '.js"></script>');
